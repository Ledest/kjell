%%
%% This is a work based on shell.erl from OTP, see license below
%%
%% -----------------------------------------------------------------
%%
%% %CopyrightBegin%
...
report_exception(Class, Severity, {Reason,Stacktrace}, RT) ->
    ...
    Enc = encoding(),
    Str = k_lib:format_exception(I, Class, Reason, Stacktrace, SF, PF, Enc),
...
local_func(rp, [A], Bs0, _Shell, RT, Lf, Ef) ->
    ...
    Cs = k_io_lib_pretty:print(V, [{column, 1},
                                    {line_length, columns()},
                                    {depth, -1},
                                    {max_chars, ?CHAR_MAX},
